  ...
  return (
    <section id="como-pedir" className="scroll-mt-24 bg-white py-16">
      <div className="mx-auto max-w-7xl px-4 sm:px-6">
        <p className="text-sm font-black uppercase tracking-[0.18em] text-[#DA291C]">Como pedir</p>
        <h2 className="mt-2 text-4xl font-black tracking-tight text-[#1D1D1D]">Pedido rapido, sem complicar.</h2>

        <div className="mt-6 grid gap-4 md:grid-cols-3">
          {steps.map((step, index) => (
            <article key={step.title} className="rounded-lg border border-zinc-100 bg-[#F5F5F5] p-6 shadow-sm">
              <span className="grid size-12 place-items-center rounded-full bg-[#FFC72C] text-lg font-black text-[#DA291C]">
                {index + 1}
              </span>
              <h3 className="mt-4 text-xl font-black text-[#1D1D1D]">{step.title}</h3>
